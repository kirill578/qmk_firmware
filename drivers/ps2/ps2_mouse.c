...
void ps2_mouse_task(void) {
    ...
    uint8_t mdp_b0; /* First byte of mouse data packet */
    ...
    if (rcv == PS2_ACK) {
        mdp_b0               = ps2_host_recv_response();
        mouse_report.buttons = mdp_b0 & PS2_MOUSE_BTN_MASK;
...
#    endif
    ...
    if (mouse_report.x || mouse_report.y || mouse_report.v || (mouse_report.buttons ^ buttons_prev)) {
#ifdef PS2_MOUSE_DEBUG_RAW
        // Used to debug raw ps2 bytes from mouse
        ps2_mouse_print_report(&mouse_report);
#endif
        buttons_prev = mouse_report.buttons;
        ps2_mouse_convert_report_to_hid(&mouse_report, mdp_b0);
...
static inline void ps2_mouse_convert_report_to_hid(report_mouse_t *mouse_report, const uint8_t mdp_b0) {
    ...
    const bool x_is_neg = mdp_b0 & (1 << PS2_MOUSE_X_SIGN);
    const bool y_is_neg = mdp_b0 & (1 << PS2_MOUSE_Y_SIGN);
    const bool x_is_ovf = mdp_b0 & (1 << PS2_MOUSE_X_OVFLW);
    const bool y_is_ovf = mdp_b0 & (1 << PS2_MOUSE_Y_OVFLW);
